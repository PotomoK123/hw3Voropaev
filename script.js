...
let togglePrice = true,
    toggleReview = true;
reviewButton.addEventListener("click", () => {

    if (toggleReview) {
        render(sortProducts(arr, "ratingRevievs"));
        toggleReview = false;
    } else {

        render(sortProducts(arr, "ratingRevievs").reverse());
        toggleReview = true;
    }
});
priceButton.addEventListener("click", () => {
    if (togglePrice) {
        render(sortProducts(arr, "price"));
        togglePrice = false;
    } else {

        render(sortProducts(arr, "price").reverse());
        togglePrice = true;
    }
});
function sortProducts(arr, value) {
    let sortedArray = [];
    sortedArray = arr.sort((prev, next) => {
        if (typeof (next[value]) === "object" && typeof (prev[value]) === "object") {
            return +prev[value].newUan.replace(/\s*\D+/g, "") > +next[value].newUan.replace(/\s*\D+/g, "") ? -1 : 0;
        }
        if (typeof (prev[value]) === "object") {
            return +prev[value].newUan.replace(/\s*\D+/g, "") > +next[value].replace(/\s*\D+/g, "") ? -1 : 0;
        }
        if (typeof (next[value]) === "object") {
            return +prev[value].replace(/\s*\D+/g, "") > +next[value].newUan.replace(/\s*\D+/g, "") ? -1 : 0;
        }

        return +prev[value].replace(/\s*\D+/g, "") > +next[value].replace(/\s*\D+/g, "") ? -1 : 0;
    });
...
function render(arr) {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        row.appendChild(col);
        col.innerText = arr[i].name;
        col = document.createElement("td");
        row.appendChild(col);
        col.innerText = arr[i].ratingRevievs;
        col = document.createElement("td");
        row.appendChild(col);
        if (typeof (arr[i].price) === "object") {
            col.innerText = arr[i].price.newUan;
        } else {
            col.innerText = arr[i].price;
        }
